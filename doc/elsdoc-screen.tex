%%
%% This is file `elsdoc-screen.tex',
...
\global\setbox\contbox=\vbox{\hsize=108mm
 ...
 \item \Hlink{frontmatter}{Frontmatter}
...
\color{Olive}%
\copyright\ 2009, Elsevier Ltd.  Bugs, feature requests,
...
Please\linebreak download the \file{elsarticle.dtx} which is the composite
...
\item \verb+1p+ --- formats the article to the look and feel of the final
  format of model 1+ journals. This is always single column style.

\item \verb+3p+ --- formats the article to the look and feel of the final
...
\item \verb+sort&compress+ sorts and compresses the
...
\item The default options loaded are \verb+a4paper+, \verb+10pt+,
  \verb+oneside+, \verb+onecolumn+\linebreak and \verb+preprint+.
...
type is provided in the next page.
\end{enumerate}
...
\begin{vquote}
 ...
 \author[rvt,focal]{V.~Apu\fnref{fn2}}
 \ead{apu.v@river-valley.com}
...
resolve the footnote marks in the frontmatter part.  Given below are
...
\begin{vquote}
\author{V.~Apu\fnref{fn2}}
 \ead{apu.v@river-valley.com}
 \address{STM Document Engineering Pvt Ltd., River Valley Campus, 
 Mepukada, Malayinkizh, Trivandrum 695571, Kerala}
\end{vquote}
\vspace*{-6pt}
...
The frontmatter part has further environments such as
abstract and keywords. These can be marked up in the following manner:
...
several options to further control graphic.
...
theorem-like environments with ease. In all commands the options
...
Roll over \tooltip{your mouse here}{mytip4} to see the typeset copy of
the above code.
Furthermore, the enhanced list environment allows one to prefix a
...
\href{http://support.river-valley.com/wiki/index.php?title=Model-wise_bibliographic_style_files}
...
\includeclip{1}{104 450 492 712}{els-1p.pdf}
\def\rulecolor{orange}
